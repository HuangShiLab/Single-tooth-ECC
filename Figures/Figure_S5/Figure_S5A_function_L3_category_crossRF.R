...
#}
fmetadata <- matrix(NA, nrow = ncol(df), ncol = 3)
fmetadata <- as.data.frame(fmetadata)
colnames(fmetadata) <- c("X", "Taxon", "Confidence")
fmetadata$X <- colnames(df)
fmetadata$Taxon <- colnames(df)
fmetadata$Confidence <- 1
...
if(any(nchar(all_wilcox_res$feature)>20)){
  all_wilcox_res<-all_wilcox_res %>% dplyr::group_by(dataset)
...
write.table(all_wilcox_res, paste0(outpath, "/all_Taxon_OTU.wilcox_res.tsv"), sep = "\t", quote = F,
...
vld_data_file <- "../../../data/637/function/KO/functions_category.l3.Abd.txt"
...
if(grepl("biom$", vld_data_file)){
  ...
  vld_df<-read.table(vld_data_file, header=T, row.names=1, sep="\t", quote="", comment.char = "", check.names = F)
}
...
vld_fmetadata <- matrix(NA, nrow = ncol(df), ncol = 3)
vld_fmetadata <- as.data.frame(vld_fmetadata)
colnames(vld_fmetadata) <- c("X", "Taxon", "Confidence")
vld_fmetadata$X <- colnames(df)
vld_fmetadata$Taxon <- colnames(df)
vld_fmetadata$Confidence <- 1
