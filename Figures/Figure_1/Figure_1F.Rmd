...
outdir <- "../../Results/Figure_1/Figure_1F"
...
taxonomic_dist_file <- "../../Results/Dist_matrix/1867_taxonomic_phylo_rPCA/1867_taxonomic_phylo_rpca_dist.txt"
functional_dist_file <- "../../Results/Dist_matrix/1867_functional_rPCA/1867_functional_rpca_dist.txt"
...
for(i in 1:length(dm_list)){
    ...
    if(is.null(names(stat_summ_list))) {
...
write.table(stat_summ_all, paste0(outdir, "/1867_Beta_diversity_summ_without_any_block.xls"), 
...
for(i in 1:length(dm_list)){
    ...
    if(is.null(names(file_list))) {
      stat_summ_list[[i]] <- stat_summ
      sink(paste0(outdir, "/1867_dist", i, ".Beta_diversity_summ_blocking_HostID.xls"));
      cat("\t");
      write.table(stat_summ, quote=FALSE,sep='\t',row.names=TRUE);
      sink(NULL)
    }
      ...
      stat_summ_list[[names(file_list)[i]]] <- stat_summ
      sink(paste0(outdir, "/1867_", names(file_list)[i], ".Beta_diversity_summ_blocking_HostID.xls"));
      cat("\t");
      write.table(stat_summ, quote=FALSE,sep='\t',row.names=TRUE);
      sink(NULL)
    }
...
stat_summ_list2 <- lapply(1:length(stat_summ_list), function(i) data.frame(feature=rownames(stat_summ_list[[i]]),
                                                             dist=names(stat_summ_list)[i], 
                                                             stat_summ_list[[i]]))
stat_summ_all<-data.frame(do.call("rbind", stat_summ_list2))

stat_summ_all

write.table(stat_summ_all, paste0(outdir, "/1867_Beta_diversity_summ_blocking_HostID.xls"), 
            quote=FALSE, sep='\t', row.names=FALSE)
```
...
for(i in 1:length(dm_list)){
    ...
    if(is.null(names(file_list))) {
      stat_summ_list[[i]] <- stat_summ
      sink(paste0(outdir, "/1867_dist", i, ".Beta_diversity_summ_blocking_HostID_Timepoint.xls"));
      cat("\t");
      write.table(stat_summ, quote=FALSE,sep='\t',row.names=TRUE);
      sink(NULL)
    }
      ...
      stat_summ_list[[names(file_list)[i]]] <- stat_summ
      sink(paste0(outdir, "/1867_", names(file_list)[i], ".Beta_diversity_summ_blocking_HostID_Timepoint.xls"));
      cat("\t");
      write.table(stat_summ, quote=FALSE,sep='\t',row.names=TRUE);
      sink(NULL)
    }
...
write.table(stat_summ_all, paste0(outdir, "/1867_Beta_diversity_summ_blocking_HostID_Timepoint.xls"), 
            quote=FALSE, sep='\t', row.names=FALSE)
```

## Plots
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
stat_summ_without_block <- read.table(paste0(outdir, "/1867_Beta_diversity_summ_without_any_block.xls"), 
                                      sep = "\t", header = T)
stat_summ_without_block$Group <- "Without_blocking"

stat_summ_blocking_hostid <- read.table(paste0(outdir, "/1867_Beta_diversity_summ_blocking_HostID.xls"), 
                                      sep = "\t", header = T)
stat_summ_blocking_hostid$Group <- "HostID_blocked"

stat_summ_blocking_hostid_timepoint <- 
  read.table(paste0(outdir, "/1867_Beta_diversity_summ_blocking_HostID_Timepoint.xls"), sep = "\t", header = T)
...
p
ggsave(paste0(outdir, "/1867_Adonis_results.pdf"), p, width = 11, height = 7)
```
