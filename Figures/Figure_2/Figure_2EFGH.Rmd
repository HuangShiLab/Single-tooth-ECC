...
taxonomic_pc_file_637 <- "../../Results/Dist_matrix/637_taxonomic_phylo_rPCA/637_taxonomic_phylo_rpca_sample_coordinates.tsv"
functional_pc_file_637 <- "../../Results/Dist_matrix/637_functional_rPCA/637_functional_rpca_sample_coordinates.tsv"

metadata_file_1867 <- "../../Data/1867/1867_metadata.txt"
taxonomic_pc_file_1867 <- "../../Results/Dist_matrix/1867_taxonomic_phylo_rPCA/1867_taxonomic_phylo_rpca_sample_coordinates.tsv"
functional_pc_file_1867 <- "../../Results/Dist_matrix/1867_functional_rPCA/1867_functional_rpca_sample_coordinates.tsv"
```
...
pc1_correlation_boxplot <- function(table, metadata, pre_ourdir) {
  ...
  mapping <- read.table("../../Data/Figure_2/Mapping_of_tooth_position.txt", sep = "\t", header = T, row.names = 1)
...
```{r, warning=FALSE}
table <- read.table(taxonomic_pc_file_637, header = T, row.names = 1)
table <- table[order(rownames(table)), ]

metadata<-read.table(metadata_file_637,header=T,sep="\t",row.names=1, quote="", comment.char="")
...
pc1_correlation_boxplot(table_h, metadata_h, paste0(outdir, "/Figure2_E1_637_taxonomy_H2H"))

metadata_h <- subset(metadata, HostGroup != "H2H")
table_h <- subset(table, rownames(table) %in% rownames(metadata_h))
pc1_correlation_boxplot(table_h, metadata_h, paste0(outdir, "/Figure2_E2_637_taxonomy_C2C"))
```
```{r, warning=FALSE}
...
metadata_h <- subset(metadata, HostGroup == "H2H")
table_h <- subset(table, rownames(table) %in% rownames(metadata_h))
pc1_correlation_boxplot(table_h, metadata_h, paste0(outdir, "/Figure2_G1_637_function_H2H"))

metadata_h <- subset(metadata, HostGroup != "H2H")
table_h <- subset(table, rownames(table) %in% rownames(metadata_h))
pc1_correlation_boxplot(table_h, metadata_h, paste0(outdir, "/Figure2_G2_637_function_C2C"))
```
...
table <- read.table(data_file, header = T, row.names = 1)
table <- table[order(rownames(table)), ]
metadata<-read.table(metadata_file_1867,header=T,sep="\t",row.names=1, quote="", comment.char="")
metadata<-metadata[which(rownames(metadata) %in% rownames(table)), ]
identical(rownames(table), rownames(metadata))
...
metadata_h <- subset(metadata, HostGroup == "H2H")
table_h <- subset(table, rownames(table) %in% rownames(metadata_h))
pc1_correlation_boxplot(table_h, metadata_h, paste0(outdir, "/Figure2_F1_1867_taxonomy_H2H"))

metadata_h <- subset(metadata, HostGroup != "H2H")
table_h <- subset(table, rownames(table) %in% rownames(metadata_h))
pc1_correlation_boxplot(table_h, metadata_h, paste0(outdir, "/Figure2_F2_1867_taxonomy_H2C_C2C"))
```
...
metadata_h <- subset(metadata, HostGroup == "H2H")
table_h <- subset(table, rownames(table) %in% rownames(metadata_h))
pc1_correlation_boxplot(table_h, metadata_h, paste0(outdir, "/Figure2_H1_1867_function_H2H"))

metadata_h <- subset(metadata, HostGroup != "H2H")
table_h <- subset(table, rownames(table) %in% rownames(metadata_h))
pc1_correlation_boxplot(table_h, metadata_h, paste0(outdir, "/Figure2_H2_1867_function_H2C_C2C"))
```
