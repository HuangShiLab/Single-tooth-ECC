...
pdf("../../Results/Figure_8/MiC/1867_rarefied_all_asv_gfc_cor_dist_heatmap.pdf",width=4.5,height=4)
...
imp_r <- read.table("../../Results/Figure_6/MiC/rarefied_taxonomy_Position_crossRF_out/all_Taxon_OTU.RF_imps.tsv",
                    sep = "\t", header = T)
most_imp_f <- subset(imp_r, imp_rank_min <= 32)$feature
df <- subset(df, rownames(df) %in% most_imp_f)
cor_dist <- 1 - cor(df)

pdf("../../Results/Figure_8/MiC/1867_rarefied_32_asv_gfc_cor_dist_heatmap.pdf",width=4.5,height=4)
pheatmap(as.matrix(cor_dist),
         clustering_method = "average",
         border_color = NA,
         treeheight_row = 50,
...
data_HHCC_gfc <- read.table("rarefied_data_HHCC_res.txt", sep = "\t", header = T)
data <- data_HHCC_gfc[, c("feature", "dataset", "generalized_logfc")]
df <- dcast(data, feature ~ dataset)
rownames(df) <- df$feature
HHCC_gfc_df <- df[, -1]

data_HHRH_gfc <- read.table("rarefied_data_HHRH_res.txt", sep = "\t", header = T)
...
ggsave("../../Results/Figure_8/MiC/rarefied_taxonomy_Position_crossRF_out/HHCC_VS_HHRH_gfc_pos_scattor_plot.pdf", gg, height = 8, width = 8)

data <- subset(data, feature %in% most_imp_f)
Taxonomy <- read.table("../../data/1867/taxonomy/GTDB_taxonomy_collaspse/taxonomy.tsv", 
                       sep = "\t", header = T, row.names = 1)
imp_r <- imp_r[order(imp_r$imp_rank_mean), ]
most_imp_f <- subset(imp_r, imp_rank_min <= 32)$feature
matched_index <- match(imp_r$feature, rownames(Taxonomy))
imp_r$Taxon <- Taxonomy[matched_index, "Taxon"]
most_imp_t <- subset(imp_r, imp_rank_min <= 32)$Taxon
matched_index <- match(data$feature, rownames(Taxonomy))
data$Taxon <- Taxonomy[matched_index, "Taxon"]
data$Taxon <- factor(data$Taxon, levels = imp_r$Taxon, ordered = T)
...
gg <- ggplot(data, aes(x = HHCC_gfc, y = HHRH_gfc)) + 
  ...
  theme(panel.grid.major = element_blank(),
        ...
        panel.background = element_blank(),
        legend.position = "none")
ggsave("../../Results/Figure_8/MiC/rarefied_taxonomy_Position_crossRF_out/HHCC_VS_HHRH_gfc_32_asv_scattor_plot.pdf", gg, height = 40, width = 40)
