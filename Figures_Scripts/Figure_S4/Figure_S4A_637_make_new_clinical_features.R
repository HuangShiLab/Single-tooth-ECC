...
p
ggsave(filename=paste0(outdir, "/FigureS5A1_637_clinical_features1.pdf"),plot=p, width=10, height=3)
...
p <- ggplot(df2, aes(x = Future_Status_Tooth, y = value)) + 
  ...
  theme(strip.text = element_text(size = 14),
        ...
        panel.background = element_blank())
p
ggsave(filename=paste0(outdir, "/FigureS5A2_637_clinical_features2.pdf"),plot=p, width=4, height=3)
