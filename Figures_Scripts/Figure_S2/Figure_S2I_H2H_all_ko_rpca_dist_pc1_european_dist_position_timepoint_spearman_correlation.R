...
same_host_timepoint_delta_pc1 <- function(df, metadata, group) {
  ...
  df <- melt(euro_dist)
  df[, c("h1", "t1", "p1", "g1")] <- metadata[df$Var1, c("HostID", "Timepoint", "Position0", "HostGroup")]
  df[, c("h2", "t2", "p2", "g2")] <- metadata[df$Var2, c("HostID", "Timepoint", "Position0", "HostGroup")]
  ...
  pdf(paste0(outpath, "/", "Fig2G_", group, "_H2H_all_ko_rpca_dist_pc1_european_dist_spearman_correlation_matrix.pdf"))
  pairs(data,
...
scatter_plot <- function(df, metadata, group) {
  ...
  pdf(paste0(outpath, "/Fig2F_", group, "_H2H_all_ko_rpca_dist_pc1_european_dist_spearman_correlation_matrix.pdf"))
  #plot(T1, T2, pch = 19) +
  plot(T1, T2, type = "n") + 
