...
stat_summ_taxo_1867_block_hostid <- read.table(paste0(outpath, 
                                                     "1867_taxonomy_dist_H2H_block_hostid.PerMANOVA_out/All_metadata", 
                                                     length(all_group), ".Beta_diversity_summ.xls"),
                                              sep='\t', header = T)
stat_summ_func_1867_block_hostid <- read.table(paste0(outpath, 
                                                     "1867_function_dist_H2H_block_hostid.PerMANOVA_out/All_metadata", 
                                                     length(all_group), ".Beta_diversity_summ.xls"),
                                              sep='\t', header = T)

stat_summ_taxo_1867_block_hostid_timepoint <- read.table(paste0(outpath, 
                                                               "1867_taxonomy_dist_H2H_block_hostid_timepoint.PerMANOVA_out/All_metadata", 
                                                               length(all_group), ".Beta_diversity_summ.xls"),
                                                        sep='\t', header = T)
stat_summ_func_1867_block_hostid_timepoint <- read.table(paste0(outpath, 
                                                               "1867_function_dist_H2H_block_hostid_timepoint.PerMANOVA_out/All_metadata", 
                                                               length(all_group), ".Beta_diversity_summ.xls"),
                                                        sep='\t', header = T)
...
pdf(paste0(outpath, "/1867_H2H_Adonis_results.pdf"), width = 22, height = 9)
